...
void Boss::TransitionIdleState()
{
	...
	state = STATE::IDLE;  // 現在の状態をIDLEに設定
	bossAnimation = BossAnimation::BOSS_IDLE;
	stateTimer = 0;  // 状態の経過時間をリセット
	stateDuration = 2.0f;  // 状態の持続時間を2秒に設定
}
...
void Boss::TransitionWalkState()
{
	...
	stateTimer = 0;  // 状態の経過時間をリセット
	stateDuration = 3.0f;  // 状態の持続時間を3秒に設定
}
...
void Boss::TransitionAttack_Tackle_State()
{
	...
	stateTimer = 0;  // 状態の経過時間をリセット
	stateDuration = 1.0f;  // 状態の持続時間を1秒に設定
}
...
void Boss::TransitionAttack_Jump_State()
{
	...
	stateTimer = 0;  // 状態の経過時間をリセット
	stateDuration = 2.5f;  // 状態の持続時間を2.5秒に設定

	chargeEffect->Play(position, 3.0f);  // チャージエフェクトを再生
}
...
void Boss::TransitionAttack_ShotStraight_State()
{
	...
	charaParam.acceleration = ACCELERATION_JUMP_SPEED;  // 加速度を設定
	rapidCount = 0;  // 連射カウントをリセット
	stateTimer = 0;  // 状態の経過時間をリセット
...
void Boss::TransitionDamageState()
{
	...
	stateTimer = 0;  // 状態の経過時間をリセット
	stateDuration = DAMAGE_STUN_DURATION;  // ダメージのスタン状態の持続時間を設定
}
