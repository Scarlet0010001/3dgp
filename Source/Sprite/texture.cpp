    ...
    D3D11_TEXTURE2D_DESC* texture2d_desc)
{
...
#else//DDS ファイルをロードしシェーダーリソースビューを生成する
    ...
    if (std::filesystem::exists(dds_filename.c_str()))
    {
        ...
        hr = CreateDDSTextureFromFile(device.Get(),
            immediate_context.Get(), dds_filename.c_str(),
    ...
    {
        hr = CreateWICTextureFromFile(device.Get(), filename, 
            resource.GetAddressOf(), shader_resource_view);
        _ASSERT_EXPR(SUCCEEDED(hr), hr_trace(hr));
    ...
    // SKY_MAP
    if (filename == L"Resources/SkyMap/captured at (0, 0, 0)/skybox.dds")
    {
