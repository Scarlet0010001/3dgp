...
void GamePad::Update(float elapsedTime)
{
	...
	// キーボードでエミュレーション
	{
		...
		if (GetAsyncKeyState('Q') & 0x8000) newButtonState |= BTN_LEFT_TRIGGER;
		if (GetAsyncKeyState('C') & 0x8000) newButtonState |= BTN_RIGHT_TRIGGER;
		if (GetAsyncKeyState('R') & 0x8000) newButtonState |= BTN_LEFT_THUMB;
		...
		if (GetAsyncKeyState('Z') & 0x8000)  newButtonState |= BTN_RIGHT_SHOULDER;
...
bool GamePad::SetVibration(float R, float L, float StopTime)
{
	isVibration = true;
	vib_time = 0.0f;
