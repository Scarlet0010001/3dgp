...
husk_particles::husk_particles(ID3D11Device* device, size_t max_particle_count) : max_particle_count(max_particle_count)
{
	...
	vertex_shader = shader<ID3D11VertexShader>::_emplace(device, "Shader/husk_particles_vs.cso", NULL, NULL, 0);
	pixel_shader = shader<ID3D11PixelShader>::_emplace(device, "Shader/husk_particles_ps.cso");
	geometry_shader = shader<ID3D11GeometryShader>::_emplace(device, "Shader/husk_particles_gs.cso");
	compute_shader = shader<ID3D11ComputeShader>::_emplace(device, "Shader/husk_particles_cs.cso");

	copy_buffer_cs = shader<ID3D11ComputeShader>::_emplace(device, "Shader/husk_particles_copy_buffer_cs.cso");
	accumulate_husk_particles_ps = shader<ID3D11PixelShader>::_emplace(device, "Shader/accumulate_husk_particles_ps.cso");
}
