...
PrimitiveRenderer::PrimitiveRenderer(ID3D11Device* device)
{
	...
	create_vs_from_cso(
		device,
		"Shader/primitive_renderer_vs.cso",
	...
	create_ps_from_cso(
		device,
		"Shader/primitive_renderer_ps.cso",
		pixelShader.GetAddressOf());

	// 定数バッファ
	primitiveConstants = std::make_unique<Constants<PrimitiveConstants>>(device);
...
void PrimitiveRenderer::AddVertex(const DirectX::XMFLOAT3& position, const DirectX::XMFLOAT4& color)
{
	...
	v.position = position;
	v.color = color;
	primitiveConstants->data.color = color;
}
	...
	D3D11_PRIMITIVE_TOPOLOGY primitiveTopology)
{
	...
	// 定数バッファ更新
	DirectX::XMStoreFloat4x4(&primitiveConstants->data.viewProjection, VP);
	primitiveConstants->Bind(dc, 0, CB_FLAG::PS_VS);
